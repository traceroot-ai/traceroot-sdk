...
class TestCredentialRefresh(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = TraceRootConfig(
            ...
            local_mode=True,  # Use local mode to avoid CloudWatch setup during init
            enable_log_console_export=False
        )
        
        # Create logger in local mode to avoid CloudWatch complications during setup
    ...
    def test_fetch_aws_credentials_caching(self):
        ...
        future_expiration = (datetime.now(timezone.utc) + timedelta(hours=2)).strftime('%Y-%m-%dT%H:%M:%SZ')
        mock_credentials = {
        ...
        with patch('traceroot.logger.requests.get', return_value=mock_response) as mock_get:
            ...
            # Second call should use cached credentials (no additional HTTP request)
    ...
    def test_fetch_aws_credentials_refresh_on_expiry(self):
        ...
        expired_time = datetime.now(timezone.utc) + timedelta(minutes=20)  # Expires in 20 minutes
        ...
        new_credentials = {
            'aws_access_key_id': 'NEWKEY123',
            'aws_secret_access_key': 'new_secret',
            'aws_session_token': 'new_token',
            'region': 'us-east-1',
            'hash': 'new-hash',
            'expiration_utc': (datetime.now(timezone.utc) + timedelta(hours=12)).strftime('%Y-%m-%dT%H:%M:%SZ'),
            'otlp_endpoint': 'https://otlp.test.com'
        }
        ...
        with patch('traceroot.logger.requests.get', return_value=mock_response):
            # Should refresh credentials because they expire in 20 minutes (< 30 minute threshold)
    ...
    def test_fetch_aws_credentials_no_refresh_when_not_expiring(self):
        """Test that credentials are not refreshed when they have plenty of time left"""
    ...
    def test_fetch_aws_credentials_force_refresh(self):
        ...
        # Mock new credentials response
        new_credentials = {
            'aws_access_key_id': 'FORCEKEY123',
            'aws_secret_access_key': 'force_secret',
            'aws_session_token': 'force_token',
            'region': 'us-east-1',
            'hash': 'force-hash',
            'expiration_utc': (datetime.now(timezone.utc) + timedelta(hours=12)).strftime('%Y-%m-%dT%H:%M:%SZ'),
            'otlp_endpoint': 'https://otlp.test.com'
    ...
    def test_refresh_credentials_success(self):
        ...
        mock_credentials = {
            'aws_access_key_id': 'REFRESHKEY123',
        ...
        with patch('traceroot.logger.requests.get', return_value=mock_response), \
             patch.object(self.logger, '_setup_cloudwatch_handler') as mock_setup:
    ...
    def test_refresh_credentials_local_mode_skip(self):
        """Test that refresh_credentials skips CloudWatch handler setup in local mode"""
        self.logger.config.local_mode = True
        
        mock_credentials = {
            'aws_access_key_id': 'LOCALKEY123',
    ...
    def test_check_and_refresh_credentials_non_local_mode(self):
        """Test that _check_and_refresh_credentials works in non-local mode"""
        self.logger.config.local_mode = False
        
        mock_credentials = {
            'aws_access_key_id': 'CHECKKEY123',
            'aws_secret_access_key': 'check_secret',
            'aws_session_token': 'check_token',
            'region': 'us-east-1',
            'hash': 'check-hash',
            'expiration_utc': (datetime.now(timezone.utc) + timedelta(hours=12)).strftime('%Y-%m-%dT%H:%M:%SZ'),
            'otlp_endpoint': 'https://otlp.test.com'
        ...
        with patch.object(self.logger, '_fetch_aws_credentials', return_value=mock_credentials) as mock_fetch:
            self.logger._check_and_refresh_credentials()
    ...
    def test_logging_methods_call_credential_check(self):
        ...
        with patch.object(self.logger, '_check_and_refresh_credentials') as mock_check, \
    ...
    def test_expiration_parsing_fallback(self):
        ...
        with patch('traceroot.logger.requests.get', return_value=mock_response):
            ...
            self.assertLess(
                abs((self.logger._credentials_expiry - expected_expiry).total_seconds()), 
                60
            )
