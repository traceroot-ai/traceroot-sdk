...
class TestTracer(unittest.TestCase):
    ...
    @patch('boto3.Session')
    def test_both_console_and_cloud_span_enabled(self, mock_boto_session, mock_fetch_credentials):
        """Test that both console and cloud span processors are added when both are enabled"""
        ...
        # Should have both SimpleSpanProcessor (console) and BatchSpanProcessor (OTLP)
    ...
    def test_only_console_span_enabled(self):
        """Test that only console span processor is added when only console is enabled"""
        provider = _initialize_tracing(
            service_name="test-service",
            github_owner="test-owner",
            github_repo_name="test-repo",
    ...
    @patch('traceroot.logger.TraceRootLogger._fetch_aws_credentials')
    @patch('boto3.Session')
    def test_only_cloud_span_enabled(self, mock_boto_session, mock_fetch_credentials):
        """Test that only cloud span processor is added when only cloud is enabled"""
        # Mock AWS credentials
        mock_fetch_credentials.return_value = None
        mock_boto_session.return_value = MagicMock()
