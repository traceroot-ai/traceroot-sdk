...
class TestLogger(unittest.TestCase):
    ...
    @patch('boto3.Session')
    def test_both_span_and_log_cloud_export_enabled(self, mock_boto_session, mock_cloudwatch_handler):
        """Test that CloudWatch handler is created when both span and log cloud export are enabled"""
        # Mock AWS session
        ...
        with patch.object(TraceRootLogger, '_fetch_aws_credentials', return_value=None):
    ...
    def test_both_span_and_log_cloud_export_disabled(self):
        """Test that no CloudWatch handler is created when both span and log cloud export are disabled"""
        config = TraceRootConfig(
            service_name="test-service",
            github_owner="test-owner",
            github_repo_name="test-repo",
        ...
        with patch.object(TraceRootLogger, '_setup_otlp_logging_handler') as mock_otlp:
            logger = TraceRootLogger(config)
        ...
        cloudwatch_handlers = [h for h in logger.logger.handlers 
                             if 'CloudWatchLogHandler' in str(type(h))]
        self.assertEqual(len(cloudwatch_handlers), 0)

    @patch('traceroot.logger.watchtower.CloudWatchLogHandler')
    @patch('boto3.Session')  
    def test_span_enabled_log_disabled(self, mock_boto_session, mock_cloudwatch_handler):
        """Test that credentials are fetched but no CloudWatch handler is created when span is enabled but log is disabled"""
        # Mock AWS session
        mock_session_instance = MagicMock()
        mock_boto_session.return_value = mock_session_instance
        ...
        with patch.object(TraceRootLogger, '_fetch_aws_credentials', return_value=None) as mock_fetch:
            logger = TraceRootLogger(config)
    ...
    def test_span_disabled_log_enabled(self):
        """Test that when span cloud export is disabled, no cloud operations occur regardless of log setting"""
        config = TraceRootConfig(
            service_name="test-service", 
            github_owner="test-owner",
            github_repo_name="test-repo",
        ...
        with patch.object(TraceRootLogger, '_fetch_aws_credentials') as mock_fetch:
            with patch.object(TraceRootLogger, '_setup_otlp_logging_handler') as mock_otlp:
                logger = TraceRootLogger(config)
    ...
    @patch('traceroot.logger.watchtower.CloudWatchLogHandler')
    @patch('boto3.Session')
    def test_credential_refresh_logic(self, mock_boto_session, mock_cloudwatch_handler):
        ...
        with patch.object(TraceRootLogger, '_fetch_aws_credentials', return_value=mock_credentials) as mock_fetch:
            logger = TraceRootLogger(config)
            initial_call_count = mock_fetch.call_count
            
            # Test that credential refresh works when span cloud export is enabled
            ...
            # Test that credential refresh is disabled when span cloud export is disabled
            logger.config.enable_span_cloud_export = False
            result = logger.refresh_credentials()
            self.assertFalse(result)

    def test_check_and_refresh_credentials_logic(self):
        """Test _check_and_refresh_credentials behavior based on export settings"""
        config = TraceRootConfig(
            service_name="test-service",
            github_owner="test-owner",
            github_repo_name="test-repo",
