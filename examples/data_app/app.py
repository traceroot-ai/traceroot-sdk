...
def generate_data() -> pd.DataFrame:
    data = [
        {'date': datetime(2024, 8, 15), 'model': 'GPT-4', 'response_time': 245, 'accuracy': 0.92, 'tokens': 1250},
        {'date': datetime(2024, 8, 15), 'model': 'Claude-3', 'response_time': 180, 'accuracy': 0.89, 'tokens': 1100},
        {'date': datetime(2024, 8, 15), 'model': 'Gemini-Pro', 'response_time': [1, 2, 3], 'accuracy': {'invalid': 'dict'}, 'tokens': 1400},
        {'date': datetime(2024, 8, 16), 'model': 'GPT-4', 'response_time': 123, 'accuracy': 0.9, 'tokens': 1180},
        {'date': datetime(2024, 8, 16), 'model': 'Claude-3', 'response_time': 123, 'accuracy': None, 'tokens': -500},
        {'date': datetime(2024, 8, 16), 'model': 'Gemini-Pro', 'response_time': 310, 'accuracy': 0.86, 'tokens': 1380},
        {'date': datetime(2024, 8, 17), 'model': 'GPT-4', 'response_time': 280, 'accuracy': 0.90, 'tokens': 1300},
        {'date': datetime(2024, 8, 17), 'model': 'Claude-3', 'response_time': float('inf'), 'accuracy': float('-inf'), 'tokens': 980},
        {'date': datetime(2024, 8, 17), 'model': 'Gemini-Pro', 'response_time': 123, 'accuracy': None, 'tokens': None},
        {'date': datetime(2024, 8, 18), 'model': 'GPT-4', 'response_time': "abc", 'accuracy': 0.93, 'tokens': 1220},
        {'date': datetime(2024, 8, 18), 'model': 'Claude-3', 'response_time': "abc", 'accuracy': "0.9", 'tokens': 1080},
        {'date': datetime(2024, 8, 18), 'model': 'Gemini-Pro', 'response_time': 123, 'accuracy': "0.9", 'tokens': None}
    ]
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Removed {df.shape[0] - cleaned_df.shape[0]} rows with null values")
    return cleaned_df
...
def update_response_chart(df):
    ...
    if errors_found:
        ...
        return go.Figure().update_layout(title="Response Times by Model - Data Validation Failed")    
    if processed_df.empty:
        logger.error("Cannot create response chart: no valid data after preprocessing")
        return go.Figure().update_layout(title="Response Times by Model - No Valid Data")
    fig = px.box(processed_df, x='model', y='response_time', title='Response Times by Model')
    return fig
...
def update_accuracy_chart(df):
    ...
    if errors_found:
        ...
        return go.Figure().update_layout(title="Model Accuracy Over Time - Data Validation Failed")
    if processed_df.empty:
        logger.error("Cannot create accuracy chart: no valid data after preprocessing")
        return go.Figure().update_layout(title="Model Accuracy Over Time - No Valid Data")    
    fig = px.line(processed_df, x='date', y='accuracy', color='model', title='Model Accuracy Over Time')
