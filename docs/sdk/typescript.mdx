...
1. Clicking the search bar category icon and select `log`.
2. Type `requestId` in the first input box.
3. Select `=` and in the second input box type `123` for this example.
4. Type Enter button to add that criteria to search. Then you can see all traces or workflows with `requestId` as `123` logging metadata in the TraceRoot UI.
