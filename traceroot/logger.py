...
class TraceIdFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if ctx and ctx.trace_id != 0:
            ...
            record.span_id = format(ctx.span_id, "016x") if ctx.span_id != 0 else "no-span"
        else:
        ...
        record.github_version = self.config.github_version
    ...
    def _get_stack_trace(self) -> str:
        ...
        for frame_info in stack[3:]:  # Skip current frame, filter frame, and logging frame
            ...
            if 'logger.py' in filename or 'tracer.py' in filename or filename.startswith('__'):
                continue
                
            relevant_frames.append(f"{filename}:{function_name}:{line_number}")
        
        return " -> ".join(reversed(relevant_frames)) if relevant_frames else "unknown"
...
class TraceRootLogger:
    ...
    def __init__(self, config: TraceRootConfig, name: Optional[str] = None):
        ...
        self.formatter = logging.Formatter(
            '%(asctime)s;%(levelname)s;%(service_name)s;%(github_version)s;%(environment)s;'
...
def get_logger(name: Optional[str] = None) -> TraceRootLogger:
    ...
    if _global_logger is None:
        raise RuntimeError("Logger not initialized. Call initialize_tracing() first.")
